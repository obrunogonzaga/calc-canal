...
async function main(): Promise<void> {
  ...
  try {
    const sql = await readFile(new URL("../migrations/0008_subscription_payments.sql", import.meta.url), "utf8");
...
void main().catch(() => {
  console.error("A migração de pagamentos recorrentes falhou.");
  process.exitCode = 1;
});
