...
Atualização de identidade: o fundador confirmou o rebrand para **Líquido** e o registro de `useliquido.com.br` e `useliquido.com`. O `.com.br` é o canonical planejado e o `.com` é secundário. Ambos permanecem estacionados na Hostinger; ainda não houve deploy nesses domínios nem cobrança em Produção. Fixtures de desenvolvimento e nomes internos antigos permanecem por compatibilidade.

| Tema | Decisão do piloto | Evidência atual | Falta para vender |
|---|---|---|---|
| Marca | **Líquido** é a identidade pública confirmada pelo fundador | Rebrand autorizado em 23/09; a decisão considera o risco de coexistência com a fintech Liquido e processos no INPI | Aplicar a identidade de forma consistente no app, documentos e gateway; manter revisão de marca/INPI antes de escalar |
...
| Autenticação e banco | Better Auth e PostgreSQL separados para Líquido | Implementados e testados localmente; CI usa banco descartável | Provisionar banco isolado, backup e restauração na #15/#16 |
...
| Domínio | `useliquido.com.br` como canonical planejado; `useliquido.com` como secundário | Fundador confirmou o registro e a disponibilidade dos dois domínios na Hostinger; ambos continuam estacionados | Publicar o Líquido no `.com.br`, redirecionar o `.com`, configurar HTTPS, monitoramento e backup na #16 |
